...
fn_regular= r'tex/Insan-Regular.ttf')
if platform == 'win':
    Window.size = (300, 600)
else:
...
####   load theme
_theme_file_path = r'tex/tar.txt'
...
class Meet_Lobby(ScreenManager):
    ...
    def change_calender(self):
        ...
        if len(self.today_lessons):
            today_lab = Button(text= self.to_arabic('دروس اليوم'),size_hint_y=None,height= label_height,
                    ...
                    background_down='',font_size=self.lessons_font_size)
            ...
            for k in range(len(self.today_lessons)):
                with self.box_canvas.canvas.before:
                    ...
                    Line(points=(wid+_shift,wid+y_pos+other_day_button, 
                                wid+_shift,50-2*wid+y_pos+other_day_button,
                                root_width-wid-_shift,50-2*wid+y_pos+other_day_button,
                                root_width-wid-_shift,wid+y_pos+other_day_button),close=True, width=wid,group='aa')
                    y_pos+=50
            ...
            for j in self.today_lessons:
                ...
                valign='middle', halign='left', font_size = self.lessons_font_size)
                ...
                valign='middle', halign='right', font_size = self.lessons_font_size)
        ...
        if len(self.other_day_lessons):
            ...
            for k in range(1,len(self.other_day_lessons)+1):
                with self.box_canvas_2.canvas.before:
                    Color(*self.lessons_canvas_color)
                    Line(points=(wid+_shift,wid+y_pos, 
                                wid+_shift,50-2*wid+y_pos,
                                root_width-wid-_shift,50-2*wid+y_pos,
                                root_width-wid-_shift,wid+y_pos),close=True, width=wid)
                    y_pos+=50
    ...
    def open_link(self, link):
        ...
        if not self.edit_mode:
            if not platform == 'win':
                link = url.split('meet.google.com')[-1]
